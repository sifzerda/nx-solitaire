...
const StockArea = memo(function StockArea({ type }) {
    ...
    return (
        <div className="flex w-fit gap-1 sm:gap-2 md:gap-3 items-center">
            ...
            {/* ---------------- STOCK ---------------- */}
            <div onClick={nextStockCard} className={`relative ${CARD_CLASS} border-2 border-dashed bg-green-500 border-green-600 rounded-md cursor-pointer overflow-hidden flex items-center justify-center touch-none`}
            ...
            <div data-dropzone="waste" className={`relative ${CARD_CLASS} rounded-md`}
                style={{ contain: "layout paint size" }}>
                {topStockCard ? (
                    ...
                    <div className="w-full h-full border-2 border-dashed border-white/40 rounded-md" />)}
            </div>
            ...
            <div className={`${CARD_CLASS} flex items-center justify-center -ml-2 sm:ml-2`}>
                {isAtEnd && (
                    <button onPointerUp={(e) => {
                        ...
                        className="group relative flex items-center justify-center rounded-sm backdrop-blur-xl border border-amber-400/90 text-amber-200 font-bold shadow-[0_8px_20px_rgba(0,0,0,0.45)] transition-all duration-100 hover:scale-[1.03] hover:border-amber-300/70 hover:text-white active:translate-y-0.75 active:shadow-[0_2px_6px_rgba(0,0,0,0.5)] cursor-pointer touch-none select-none"
                        style={{width: "70%", height: "70%", zIndex: 100, fontSize: "calc(var(--card-width) * 0.45)",}}>
                        {/* Bottom rim */}
                        <div className="absolute inset-x-0 -bottom-1 h-2 rounded-b-sm bg-amber-900/40 transition-all duration-100 group-active:h-0 group-active:bottom-0" />

                        {/* Glass highlight */}
                        <div className="absolute inset-0 rounded-sm bg-linear-to-b from-white/15 via-white/5 to-transparent pointer-events-none" />
